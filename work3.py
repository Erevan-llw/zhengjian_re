...
files = f'D:\\Liour_computer project\\python_project_idea\\zhengjian_re\\_file_pdf'
        ...
        for page in doc.get_pages():  # doc.get_pages() 获取page列表
            ...
            for x in layout:
                if (isinstance(x, LTTextBoxHorizontal)):
                    # 需要写出编码格式
                    with open(f'_text_file\\{pdf_cast}.txt', 'a', encoding='utf-8') as f:
                        results = x.get_text()
                        f.write(results + '\n')
...
if __name__ == '__main__':
    ...
    for pdf_cast in file_list:
        if '(1)' not in pdf_cast:
            path = f"D:\\Liour_computer project\\python_project_idea\\zhengjian_re\\_file_pdf\\{pdf_cast}"
            print(f'{pdf_cast}')
            pdf_cast = pdf_cast[:6]
            parse()
